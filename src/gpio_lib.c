...
int gpio_reg_init(struct gpio_reg * reg, unsigned int pin) {
    unsigned int bank = GPIO_BANK(pin);
    unsigned int num = GPIO_NUM(pin);
    unsigned int cfg_index = GPIO_CFG_INDEX(pin);
    unsigned int cfg_offset = GPIO_CFG_OFFSET(pin);
    unsigned int pull_index = GPIO_PULL_INDEX(pin);
    unsigned int pull_offset = GPIO_PULL_OFFSET(pin);
    if(SUNXI_PIO_BASE == 0)
    {
        return 0;
    }
